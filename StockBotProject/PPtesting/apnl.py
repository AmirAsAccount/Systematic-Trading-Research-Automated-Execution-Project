"""
apnl.py  —  Alt P&L Backtest  [Price Compression + Bullish Volume Divergence]

No HMM, no Cox, no covariate.py outputs required.
Reads price cache directly (same cache/ parquets as covariate.py).
...
def load_prices(resample_to=None):
    """
    Load full OHLCV from cache/ parquets for all tickers.
    Optionally resample in memory to a coarser timeframe.
    ...
    for ticker in tickers:
        chunks = []
        for path in [CACHE_DIR / f"{ticker}_1h.parquet",
                     CACHE_DIR / f"{ticker}_15m_recent.parquet"]:
            if not path.exists():
                continue
            try:
        ...
        if resample_to and resample_to != "15m":
            ...
            if rule:
                combined = combined.resample(rule, closed="left", label="left").agg(
                    {"Open": "first", "High": "max",
...
def run_trades(prices, horizon=DEFAULT_HORIZON,
    ...
    (bars after the train_frac split point, same convention as covariate.py).
    """
    trade_rows = []
    ...
    for ticker, df in prices.items():
        ...
        for t in range(nt - 1):
            ...
            for h in range(1, horizon + 1):
                ...
                if check >= nt:
                    break
                # check intrabar against high/low for target/stop
...
def print_summary(stats, horizon, vol_mult, tf, profit_target, stop_loss):
    bm = _BAR_MINUTES.get(tf, 15)
    log(f"\n{'='*65}")
    log("  APNL SUMMARY  [TEST SET ONLY]")
    log(f"{'='*65}")
    if not stats:
...
def main():
    parser = argparse.ArgumentParser(
        description="Alt backtest: price compression + bullish volume divergence")
    parser.add_argument("--horizon",        type=int,   default=DEFAULT_HORIZON)
    parser.add_argument("--vol-mult",       type=float, default=DEFAULT_VOL_MULT,
                        help="Volume must exceed vol_mult x vol_ma to signal")
    ...
    parser.add_argument("--compress-mult",  type=float, default=DEFAULT_COMPRESS_MULT,
                        help="Bar range must be < compress_mult x ATR")
    parser.add_argument("--atr-thresh",     type=float, default=DEFAULT_ATR_THRESH,
                        help="ATR/Close must be < this to qualify as compressed")
    parser.add_argument("--profit-target",  type=float, default=DEFAULT_PROFIT_TARGET,
                        help="Profit target as decimal (0=disabled)")
    parser.add_argument("--stop-loss",      type=float, default=DEFAULT_STOP_LOSS,
                        help="Stop loss as decimal (0=disabled)")
    parser.add_argument("--train-frac",     type=float, default=0.60)
    parser.add_argument("--resample-to",    default=None,
                        help=f"Resample in memory: {list(_RESAMPLE_RULES.keys())}")
    parser.add_argument("--symbols",        nargs="*",  default=None)
    parser.add_argument("--no-sweep",       action="store_true")
    args = parser.parse_args()

    tf       = args.resample_to if args.resample_to in _RESAMPLE_RULES else "15m"
    bar_mins = _BAR_MINUTES.get(tf, 15)

    log("=" * 65)
    log("  apnl.py  —  Price Compression + Bullish Volume Divergence")
    log("=" * 65)
    ...
    prices = load_prices(resample_to=args.resample_to)
    log(f"  Loaded {len(prices)} symbols")

    if args.symbols:
        prices = {k: v for k, v in prices.items() if k in args.symbols}
        log(f"  Filtered to {len(prices)} symbols: {args.symbols}")
    ...
    log(f"\n{'─'*65}")
    if not trades.empty:
        trades.to_csv("apnl_trades.csv", index=False)
        log("  Saved → apnl_trades.csv")
        sym_df.to_csv("apnl_by_symbol.csv")
        log("  Saved → apnl_by_symbol.csv")
    if not sweep_df.empty:
        sweep_df.to_csv("apnl_by_threshold.csv")
        log("  Saved → apnl_by_threshold.csv")

    with open("apnl_summary.txt", "w") as f:
        f.write("ALT P&L BACKTEST SUMMARY\n")
