"""
backtestc.py  —  Signal Lead-Time & Threshold Backtest  [CRYPTO VERSION]

Identical logic to backtest.py, adapted for covariatec.py outputs:
  - Reads crypto_covariate_states.csv  (column: 'symbol' not 'ticker')
  - Reads crypto_events.csv            (column: 'symbol' not 'ticker')
  - Writes crypto_backtest_*.csv
...
RESULTS_CSV         = "crypto_covariate_results.csv"
...
def load_data():
    ...
    # ── integrity check: p_burst must be in [0, 1] ───────────────────────────
    ...
    if max_p > 1.0001 or min_p < -0.0001:
        ...
        log("  Re-run covariatec.py (fixed version) which uses")
    ...
    if "split" in states.columns:
        ...
        log("  Re-run covariatec.py to get the split column.")

    # covariatec.py uses 'symbol'; normalise to 'ticker' internally
    ...
    for ticker, ev_grp in events.groupby("ticker"):
        ...
        for tau in thresholds:
            ...
            for sb in signal_bars:
                # entry at t+1, check t+1 .. t+horizon
                entry_bar = sb + 1
                hit = any(
...
def continuation_backtest(states, events, cont_window=DEFAULT_CONT_WINDOW):
    ...
    for ticker, ev_grp in events.groupby("ticker"):
        ...
        for trans_bar in burst_transitions:
            # entry is at t+1 (open of bar after transition)
...
def print_threshold(thr_df, horizon):
    ...
    for ticker in thr_df["ticker"].unique():
        ...
        log(f"  {ticker:<12}  τ={best['threshold']:.2f}  "
...
def print_continuation(cont_df):
    ...
    log(f"  {'Ticker':<14} {'Trans':>6} {'HitRate':>8} "
...
def main():
    ...
    log("=" * 65)
    log("  backtestc.py  —  Signal Backtest  [CRYPTO]")
    log("=" * 65)
    ...
    log(f"  Evaluation    : TEST SET ONLY (split=='test' from covariatec.py)")
    log("=" * 65)
    ...
    # ── save ──────────────────────────────────────────────────────────────────
    lt_df.to_csv("crypto_backtest_lead_time.csv", index=False)
    thr_df.to_csv("crypto_backtest_threshold.csv", index=False)
    cont_df.to_csv("crypto_backtest_continuation.csv", index=False)
    cont_detail.to_csv("crypto_backtest_continuation_detail.csv", index=False)

    sig_df = build_signal_log(states, events,
                               tau=args.signal_tau, horizon=args.horizon)
    sig_df.to_csv("crypto_backtest_signals.csv", index=False)

    # Save summary text
    with open("crypto_backtest_summary.txt", "w") as f:
        f.write("CRYPTO BACKTEST SUMMARY\n")
    ...
    log("\n  Saved → crypto_backtest_lead_time.csv")
    log("  Saved → crypto_backtest_threshold.csv")
    log("  Saved → crypto_backtest_continuation.csv")
    log("  Saved → crypto_backtest_continuation_detail.csv")
    log("  Saved → crypto_backtest_signals.csv")
    log("  Saved → crypto_backtest_summary.txt")
