#!/usr/bin/env python3
"""
backtest.py  —  Bullish Volume Divergence via swing point detection.

Exit priority:
  1. PRIMARY   — price closes above sw1 price (divergence complete)
  2. SECONDARY — bearish volume divergence on swing highs
  3. FALLBACK  — end of data
"""
...
# ── Entry ─────────────────────────────────────────────────────────────────────
VOL_SPIKE_MULT = 4.0    # ↓ was 6.0 — more signals
AVG_VOL_RATIO  = 1.2    # ↓ was 1.5 — more signals
CONTEXT_BARS   = 3

# ── Exit ──────────────────────────────────────────────────────────────────────
OBV_DROP_MULT            = 0.97
USE_DIV_COMPLETION_EXIT  = True
...
def check_entry(window: pd.DataFrame) -> tuple[bool, float]:
    ...
    fired = bool(price_ll and vol_spike and avg_vol_up)
    return fired, float(close.iloc[sw1]) if fired else 0.0
...
def find_exit(bars: pd.DataFrame, entry_idx: int, sw1_price: float) -> dict:
    ...
    for i in range(1, len(bars_after)):
        if USE_DIV_COMPLETION_EXIT and closes[i] > sw1_price:
            exit_idx, exit_reason = i, 'divergence_complete'
            break

        lo = i - SWING_N
        hi = i + SWING_N + 1
        if lo >= 0 and hi <= len(bars_after):
            ...
            if len(confirmed_highs) >= 2:
                ...
                if sh2[1] > sh1[1] and sh2[2] < sh1[2] * OBV_DROP_MULT:
                    exit_idx, exit_reason = sh2[0], 'bearish_vol_divergence'
                    break
...
def scan_pair(pair: str, path: Path, lookback: int) -> list[dict]:
    ...
    for entry_idx in range(lookback, len(bars) - 2):
        ...
        fired, sw1_price = check_entry(window)
        if not fired:
            continue
        exit_info    = find_exit(bars, entry_idx, sw1_price)
...
def main():
    ...
    if not parquet_files:
        print(f"No parquet files in {CACHE_DIR.resolve()}/ — run crypto_fetch.py first.")
        return
    ...
    print(f"Entry: VOL_SPIKE_MULT={VOL_SPIKE_MULT}  AVG_VOL_RATIO={AVG_VOL_RATIO}\n")
    ...
    if not all_trades:
        print("No signals. Try lowering VOL_SPIKE_MULT or AVG_VOL_RATIO.")
        return
